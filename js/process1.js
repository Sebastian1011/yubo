var process={
	...
	animate:function(obj,prop,val,spd,type,fn){
		...
		obj[prop+'Timer']=setInterval(function(){
			if(opa&&t<d){process.opa(obj,Math[m](process[st](++t,b,c,d)));}
			else if(!opa&&t<d){obj.style[prop]=Math[m](process[st](++t,b,c,d))+'px';}
	...
	setting:function(par){
		if(window.attachEvent){(function(){try{process.$(par.id).className=par.style;process.initCSS(par);}catch(e){setTimeout(arguments.callee,0);}})();window.attachEvent('onload',function(){process[par.style](par)});}
　　		else{document.addEventListener("DOMContentLoaded",function(){process.$(par.id).className=par.style;process.initCSS(par);},false);window.addEventListener('load',function(){process[par.style](par)},false);}
	},
