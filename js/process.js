process.extend({
	mF_classicHB:function(par){//*********************经典怀旧系列二--海报风格******************
		...
		eval(this.switchMF(function(){
			process.stop(pic[index]).stop(txt[index]);
			...
			process.slide(num[index],{width:16},10);num[index].className='';
		},function(){
			process.fadeIn(pic[next],20).slide(pic[next],{width:par.width,height:par.height,top:0,left:0},20)
			process.slide(txt[next],{top:0},20);
			process.slide(num[next],{width:26},10);num[next].className='current';
		}))
...
process.setting({
	...
	width:80,//宽(大图)，注意:整个焦点图的宽等于或大于图片宽
	height:80//高(大图)，注意同上
});
